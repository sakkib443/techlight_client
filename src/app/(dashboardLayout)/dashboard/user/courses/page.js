            ...
            {filteredEnrollments.length === 0 ? (
                <div className={`py-20 text-center rounded-2xl border-2 border-dashed ${isDark
                    ...
                    {!searchTerm && (
                        <Link
                            href="/courses"
                            className="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-gradient-to-r from-[#7A85F0] to-[#7A85F0] text-white rounded-xl font-bold text-sm shadow-md shadow-[#7A85F0]/10 hover:scale-105 transition-all"
                        >
                        ...
                        >
                            ...
                            <div className="relative h-44 overflow-hidden">
                                ...
                                <div className="absolute top-3 right-3 px-2.5 py-1 bg-white/90 backdrop-blur-sm rounded-lg text-[10px] font-black shadow-sm">
                                    <span className="text-[#7A85F0]">{enroll.progress || 0}%</span>
                                </div>
                            ...
                            <div className="p-4 space-y-4">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className={`h-1.5 w-full rounded-full overflow-hidden ${isDark ? 'bg-slate-700' : 'bg-slate-100'}`}>
                                        <div
                                            className="h-full bg-gradient-to-r from-[#7A85F0] to-[#7A85F0] rounded-full transition-all duration-500"
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div className={`p-2 rounded-xl text-center ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
                                        <FiLayers className="text-[#7A85F0] mx-auto mb-1" size={12} />
                                        <p className={`text-[10px] font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                                            {enroll.completedLessons || 0} Lessons
                                        </p>
                                    </div>
                                    <div className={`p-2 rounded-xl text-center ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
                                        <FiClock className="text-[#7A85F0] mx-auto mb-1" size={12} />
                                        <p className={`text-[10px] font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                                ...
                                <Link
                                    ...
                                    className="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-gradient-to-r from-[#7A85F0] to-[#c41e18] text-white rounded-xl font-bold text-xs shadow-md shadow-[#7A85F0]/10 hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all"
                                >
                    ...
                    {filteredEnrollments.map((enroll) => (
                        <div key={enroll._id} className={`flex items-center gap-4 p-4 transition-colors ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-50'}`}>
                            ...
                            <div className="text-right shrink-0">
                                <p className="text-sm font-black text-[#7A85F0]">{enroll.progress || 0}%</p>
