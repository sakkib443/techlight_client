...
export default function UserNotificationsPage() {
    ...
    const getNotificationIcon = (type) => {
        switch (type) {
            case 'course': return <FiBookOpen className="text-[#E31E27]" size={18} />;
            case 'certificate': return <FiAward className="text-[#E31E27]" size={18} />;
            ...
            <div className={`${cardClass} p-6`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#E31E27] to-[#c41e18] flex items-center justify-center text-white shadow-lg">
                            <FiBell size={28} />
                        </div>
                        <div>
                    ...
                    {unreadCount > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-[#E31E27]/10 text-[#E31E27] rounded-xl font-medium hover:bg-[#E31E27]/20 transition-all text-sm"
                        >
                            <FiCheck size={16} />
                            Mark all as read
                        </button>
                    ...
                    filteredNotifications.map((notif) => (
                        <div
                            ...
                            className={`flex items-start gap-4 p-5 transition-colors ${!notif.read
                                ? isDark ? 'bg-[#E31E27]/5' : 'bg-[#E31E27]/5'
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        <p className={`font-semibold ${isDark ? 'text-white' : 'text-gray-800'}`}>
                                            {notif.title}
                                            {!notif.read && (
                                                <span className="ml-2 w-2 h-2 bg-[#E31E27] rounded-full inline-block"></span>
                                            )}
                                        </p>
                                        ...
                                        {!notif.read && (
                                            <button
                                                ...
                                                className={`p-2 rounded-lg transition-colors ${isDark
                                                    ? 'bg-slate-700 text-slate-400 hover:text-[#E31E27]'
                                                    : 'bg-gray-100 text-gray-500 hover:text-[#E31E27]'
                                                    }`}
                                                title="Mark as read"
                                            >
                                                <FiCheck size={16} />
